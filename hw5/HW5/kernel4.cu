...
//     int *host_res = (int *)malloc(resY * pitch_in_byte);
...
__global__ void mandelKernel(float lowerX, float lowerY, float stepX, float stepY, int resX, int resY, int *img, int maxIterations)
{
    // To avoid error caused by the floating number, use the following pseudo code
    //
    int thisX = blockIdx.x * blockDim.x + threadIdx.x; // for i
...
void hostFE(float upperX, float upperY, float lowerX, float lowerY, int *img, int resX, int resY, int maxIterations)
{
    ...
    float stepX = (upperX - lowerX) / resX;
    float stepY = (upperY - lowerY) / resY;
    int *host_res = (int *)malloc(resY * resX * sizeof(int));
    int *kernel_res;
    cudaMalloc(&kernel_res, resY * resX * sizeof(int));

    dim3 threadPerBlock(BLOCK_SIZE, BLOCK_SIZE);
    dim3 numblock(resX / threadPerBlock.x, resY / threadPerBlock.y);
    mandelKernel<<<numblock, threadPerBlock>>>(lowerX, lowerY, stepX, stepY, resX, resY, kernel_res, maxIterations);

    cudaMemcpy(host_res, kernel_res, resY * resX * sizeof(int), cudaMemcpyDeviceToHost);
    for (int j = 0; j < resY; j++)
    {
        for (int i = 0; i < resX; i++)
        {
            img[j * resX + i] = host_res[j * resX + i];
        }
    }
    free(host_res);
    cudaFree(kernel_res);
